...
const App: React.FC = () => {
  ...
  const handleUpdateGeneration = async (updatedAsset: GeneratedAsset) => {
      ...
      if (currentGenerationId && formDataState && user) {
          await updateGeneration(currentGenerationId, updatedAsset, formDataState, user.id);
      }
  };
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      try {
        const saved = await saveGeneration(formData, placeholderResult, user.id);
        if (saved) {
           draftId = saved.id;
        ...
        console.warn("Initial save failed, will try again later", e);
      }
      ...
      if (draftId && user) {
        ...
        const saved = await saveGeneration(formData, finalResult, user.id);
  ...
  return (
    <div className="min-h-screen pb-20 animate-in overflow-x-hidden relative text-slate-900 bg-[#f8fafc] w-full max-w-[100vw] dark:bg-slate-950 dark:text-slate-100">
      ...
      {notification && (
        <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-top-4 w-[90%] max-w-sm px-4">
             <div className="bg-white/90 backdrop-blur-md border border-emerald-500/20 text-emerald-600 px-4 py-3 rounded-xl shadow-xl flex items-center gap-3 justify-center dark:bg-slate-800/90 dark:border-emerald-500/30 dark:text-emerald-400">
        ...
        <header className="flex items-center justify-between mb-6 md:mb-10 relative z-20">
          <div className="flex items-center gap-3 md:gap-4 max-w-[70%]">
            ...
            <div className="min-w-0">
              <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-none truncate dark:text-white">
                UGC<span className="text-brand-500">Director</span>
              </h1>
              <div className="flex flex-wrap items-center gap-2 mt-1">
                <span className="text-[9px] md:text-[10px] bg-slate-100 text-slate-500 px-1.5 md:px-2 py-0.5 rounded uppercase tracking-wider font-bold border border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700">v3.0 (Feb 2026)</span>
                <span className="flex h-2 w-2 relative shrink-0">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
                
                <span className="hidden sm:flex items-center gap-1 text-[10px] bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded uppercase tracking-wider font-bold border border-emerald-200 whitespace-nowrap dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-900/30">
          ...
          <div className="flex items-center gap-2 md:gap-3 shrink-0">
            ...
            <button 
                ...
                className="flex items-center gap-2 p-2.5 md:p-2 rounded-xl md:rounded-lg bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 transition-all hover:text-brand-600 shadow-sm active:scale-95 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400 dark:hover:text-brand-400"
                title="API Settings"
            >
            ...
            <button 
                ...
                className={`flex items-center gap-2 p-2.5 md:px-4 md:py-2 rounded-xl md:rounded-lg border transition-all shadow-sm active:scale-95 ${showHistory ? 'bg-brand-600 border-brand-500 text-white' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-brand-600 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400 dark:hover:text-brand-400'}`}
                title="History"
            >
            ...
            <button 
                ...
                className="flex items-center gap-2 p-2.5 md:p-2 rounded-xl md:rounded-lg bg-white border border-slate-200 text-slate-500 hover:bg-red-50 hover:text-red-600 transition-all shadow-sm active:scale-95 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-red-900/20 dark:hover:text-red-400"
                title="Logout"
            >
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start">
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-6">
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-brand-200 to-purple-200 rounded-[2rem] opacity-50 blur-lg transition duration-500 dark:from-brand-900 dark:to-purple-900 dark:opacity-30"></div>
              <div className="relative bg-white/50 rounded-[1.75rem] p-1 border border-white/50 dark:bg-slate-900/50 dark:border-slate-800">
                 <div className="bg-white/40 rounded-[1.5rem] p-3 md:p-5 overflow-hidden dark:bg-slate-900/40">
                    <InputForm 
            ...
            {loading && (
              <div className="fixed inset-0 bg-white/95 backdrop-blur-xl z-[60] flex items-center justify-center lg:relative lg:inset-auto lg:bg-transparent lg:backdrop-blur-none lg:z-auto p-4 dark:bg-slate-950/95">
                 <div className="w-full max-w-sm lg:max-w-none glass-panel p-6 rounded-3xl border border-brand-500/30 shadow-2xl relative overflow-hidden bg-white dark:bg-slate-900 dark:border-slate-700">
                    ...
                    <div className="space-y-5">
                        <div className={`flex items-center gap-3 text-sm transition-all ${loadingStage !== 'analyzing' ? 'text-emerald-500 opacity-50' : 'text-slate-900 font-bold scale-105 origin-left dark:text-white'}`}>
                        ...
                        <div className={`flex items-center gap-3 text-sm transition-all ${loadingStage === 'drafting' ? 'text-slate-900 font-bold scale-105 origin-left dark:text-white' : (result ? 'text-emerald-500 opacity-50' : 'text-slate-400 dark:text-slate-600')}`}>
                        ...
                        <div className={`flex items-center gap-3 text-sm transition-all ${loadingStage === 'finalizing' ? 'text-slate-900 font-bold scale-105 origin-left dark:text-white' : 'text-slate-400 dark:text-slate-600'}`}>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider dark:text-slate-400">Intelligence Engine</span>
                            <div className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] font-mono border bg-brand-50 text-brand-600 border-brand-200 dark:bg-brand-900/20 dark:text-brand-400 dark:border-brand-900/30`}>
            ...
            {error && (
               <div className="bg-red-50 border border-red-200 p-4 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2 shadow-sm dark:bg-red-900/20 dark:border-red-900/30">
                  <Info className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                  <div>
                     <h3 className="text-red-700 font-bold text-sm dark:text-red-400">Generation Failed</h3>
      ...
      {/* History Sidebar */}
      <div 
        className={`fixed inset-y-0 right-0 w-full md:w-[400px] bg-white border-l border-slate-200 shadow-2xl transform transition-transform duration-300 ease-in-out z-[80] ${showHistory ? 'translate-x-0' : 'translate-x-full'} dark:bg-slate-900 dark:border-slate-800`}
      >
         <div className="flex flex-col h-full safe-area-bottom">
            <div className="p-5 md:p-6 border-b border-slate-100 flex items-center justify-between bg-white/50 backdrop-blur safe-area-top dark:bg-slate-900/50 dark:border-slate-800">
               ...
               <button onClick={() => setShowHistory(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors dark:hover:bg-slate-800">
               ...
               {loadingHistory ? (
                   ...
                   history.map((item) => (
                      <div 
                        ...
                        className="group bg-white hover:bg-white border border-slate-200 hover:border-brand-500/50 p-4 rounded-xl cursor-pointer transition-all relative shadow-sm hover:shadow-md active:scale-[0.99] dark:bg-slate-800 dark:border-slate-700 dark:hover:border-brand-500/50"
                      >
                         <div className="flex justify-between items-start mb-2 pr-6">
                            <span className="text-xs font-bold text-brand-600 bg-brand-50 px-2 py-0.5 rounded border border-brand-100 dark:bg-brand-900/20 dark:text-brand-400 dark:border-brand-900/30">{item.brand_name}</span>
                            <span className="text-[10px] text-slate-400 flex items-center gap-1">
                               <Clock className="w-3 h-3" />
                               {new Date(item.created_at).toLocaleDateString()}
                            </span>
                         </div>
                         <h3 className="text-sm font-bold text-slate-800 mb-1 line-clamp-2 dark:text-slate-200">{item.output_plan.concept_title}</h3>
                         <div className="flex items-center justify-between text-xs text-slate-500 mt-2 dark:text-slate-400">
                         ...
                         <button 
                            ...
                            className="absolute top-2 right-2 p-2 md:p-1.5 z-10 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors md:opacity-0 md:group-hover:opacity-100 dark:hover:bg-red-900/20 dark:hover:text-red-400"
                            title="Delete"
                         >
