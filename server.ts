...
const R2_API_TOKEN = process.env.R2_API_TOKEN || "azTPfjWeJOp8j3RY1KLovBVsg2Yu4SRmni6guY-z"; // Using same token for simplicity if it has permissions
...
async function initDb() {
  ...
  try {
    ...
    await executeD1(createUsersTableSQL);
...
app.post("/api/auth/register", async (req, res) => {
  ...
  // In production, use bcrypt. For prototype with standard Node crypto:
  const hashedPassword = crypto.createHash('sha256').update(password).digest('hex');

  const sql = `INSERT INTO users (id, email, password) VALUES (?, ?, ?)`;
  
  try {
    await executeD1(sql, [id, email, hashedPassword]);
...
app.post("/api/auth/login", async (req, res) => {
  ...
  try {
    const result = await executeD1(sql, [email, hashedPassword]);
